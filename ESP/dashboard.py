...
class temp(customtkinter.CTkFrame):
    def __init__(self, *args, master, header_name="Temperatuur", **kwargs):
        ...
        self.button = customtkinter.CTkButton(self, text='Start met lezen', command=threading.Thread(target=self.update_temp).start)
    ...
    def update_temp(self):
        while True:
            ...
            temp = f.read().strip()
            self.label.configure(text=f"Temperatuur : {temp}°C")
            sleep(2)
